...
file_path = '~/masters_thesis_stevedan/masters-thesis/code/control/simulation/simulation_results/main_template/';
file_name = 'template_test';
n_data = 9;
...
controller_gains.c3_delta = 0.01;
...
for i = 1:n_agv
    swam_agv(i) = droneSecondOrderModel(q_agv(:,i), p_agv(:,i),a_agv(:,i), N, agv, i, true, false, noise.vx, noise.vy);
end
...
while ~stop_test
    ...
    for i = 1:n_agv
        ...
        % update input
        [input, input_vec ] = controller_agv(i).controller(q_agv,p_agv,ref,obs_pos,wp.ori);
        % update_state
    ...
    if norm(centroid-ref.q) < 0.02
        if wp.ind == wp.size
            counter = counter + dt;
            error =  (norm(centroid -ref.q));
            wp.buffer = [wp.buffer(2:end), error];
%             mean(wp.buffer)
            if counter > 30 % wait for seconds more mean(wp.buffer) < 0.1
                % stop all agv
                for r = 1:n_agv
